...
#[derive(Clone)]
struct Table(std::sync::Arc<Sharded<lru_cache::LruCache<u64, Arc<()>>, u64>>);

impl Collection for Table {
    type Handle = Self;
    fn with_capacity(capacity: usize) -> Self {
        let mut inner = vec![lru_cache::LruCache::new(capacity / 128); 128];
...
impl CollectionHandle for Table {
    ...
    fn get(&mut self, key: &Self::Key) -> bool {
        self.0.index(key).write().get_mut(key).cloned().is_some()
    }
